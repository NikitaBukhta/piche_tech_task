...
namespace base::modifiers {
   ...
   * @param keep_ratio - should we keep ratio? If true, should be keept the width or height where is min scale value;
   */
    virtual void update_img_path(const std::wstring& image_path) = 0;
    virtual void resize_image(const std::wstring img_path, std::int32_t width, std::int32_t height, bool keep_ratio = false) = 0;
