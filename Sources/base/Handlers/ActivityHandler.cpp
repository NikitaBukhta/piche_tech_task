...
void ActivityHandler::set_event_handler(
    std::weak_ptr<utils::EventHandler> val) {
  _event_handler = val.lock();
}

void ActivityHandler::init() {
  _event_handler->subscribe(
      this, {utils::ISubscriber::Event::UserInteractionHandled});
}

void ActivityHandler::run() { start_timer(); }

void ActivityHandler::stop() { stop_timer(); }
