...
ScreenshotManager::ScreenshotManager() {
  ...
  _image_modifier = std::make_unique<platform::modifiers::ImageModifierSpecific>();
}
...
void ScreenshotManager::run() {
  ...
  _image_modifier->update_img_path(configuration::config::SCREENSHOT_COMMON_NAME);
  _image_modifier->resize_image(configuration::config::SCREENSHOT_COMMON_NAME, configuration::config::MODIFIED_NEW_IMG_WIDTH, configuration::config::MODIFIED_NEW_IMG_HEIGHT, true);
